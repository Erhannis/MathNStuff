 ...
 * generics. </p>
 * 
 * Modified somewhat from the copy in Apache Commons Lang.
 *
 * @since 3.0
 */
public class TypeUtils {
     ...
     * 
     * Modifications:
     * 1. If a TypeVariable cannot be further resolved, it will be returned (rather than null).
     *
    ...
    public static Type unrollVariables(Map<TypeVariable<?>, Type> typeArguments, final Type type) {
        ...
        if (containsTypeVariables(type)) {
            if (type instanceof TypeVariable<?>) {
                Type result = unrollVariables(typeArguments, typeArguments.get(type));
                if (result == null) {
                  return type;
                } else {
                  return result;
